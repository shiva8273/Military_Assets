...
export default function Assignments() {
  ...
  const validate = () => {
    ...
    if (!form.assigned_to.trim())
      errs.assigned_to = "Personnel / Unit name is required";
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (val, row) =>
        val || row.assigned_by?.username || row.created_by?.username || "—",
    },
  ];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Track equipment issued to military personnel and operational units
          </p>
        </div>
      ...
      >
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            />
            {errors.assigned_to && (
              <p className="form-error">{errors.assigned_to}</p>
            )}
          </div>
          <div>
