...
export default function Transfers() {
  ...
  const validate = () => {
    ...
    if (!form.destination_base)
      errs.destination_base = "Destination Base is required";
    if (
      form.source_base &&
      form.destination_base &&
      String(form.source_base) === String(form.destination_base)
    ) {
  ...
  const getBaseName = (id) =>
    bases.find((b) => String(b.id) === String(id))?.name || `Base #${id}`;
  const getEqName = (id) =>
    eqTypes.find((e) => String(e.id) === String(id))?.name ||
    `Equipment #${id}`;
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (val, row) =>
        val || row.source_base?.name || row.source_base || "—",
    },
    {
      ...
      sortable: true,
      render: (val, row) =>
        val || row.destination_base?.name || row.destination_base || "—",
    },
    {
    ...
    },
    {
      ...
      sortable: true,
      render: (val, row) =>
        val || row.initiated_by?.username || row.created_by?.username || "—",
    },
    {
      label: "Date",
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Transfer military equipment atomically between locations
          </p>
        </div>
      ...
      <Modal
        ...
        title="Initiate Equipment Transfer"
      >
        <form onSubmit={handleOpenConfirm} className="p-6 space-y-4">
          <div>
            ...
            {role === "ADMIN" ? (
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, source_base: e.target.value })
                }
              ...
              <input
                type="text"
                value={getBaseName(form.source_base)}
                disabled
                className="form-input opacity-60 cursor-not-allowed"
              />
            )}
            {errors.source_base && (
              <p className="form-error">{errors.source_base}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, destination_base: e.target.value })
              }
            ...
            {errors.destination_base && (
              <p className="form-error">{errors.destination_base}</p>
            )}
          </div>
